 ...
 *
 * <p>The anchor encodes a consistent triple: the ET value at a known wall-clock moment, together
 * with the time rate in effect. From it, the current ET is always computable as:
 *
 * <pre>
 *   currentET = anchorET + timeRate * (wallNow - anchorWall)
 * </pre>
 *
 * <p>The entire record is replaced atomically via {@link java.util.concurrent.atomic.AtomicReference}
 * whenever the rate changes, ET is set explicitly, or the simulation resumes from pause. No field
