 ...
 * {@link AtomicReference} and replaced atomically on every rate change, {@link #setET}, or resume
 * from pause, guaranteeing that the JME render thread always reads a self-consistent triple.
 *
 * <h3>Pause model</h3>
 *
 * <p>When paused, {@code pausedWallTime} is set to the wall-clock instant at which pause was
 * requested. {@link #wallNow()} returns this clamped value so ET stops advancing. The anchor is
 * not changed during pause; it is replaced atomically on resume using the clamped wall time as the
 * new {@code anchorWall}, so resuming produces no ET jump.
 *
 * <h3>Threading</h3>
 *
 * <ul>
 *   <li>{@link #advance()} — called <em>only</em> from the JME render thread each frame.
 *   <li>{@link #setPaused}, {@link #setTimeRate}, {@link #setET}, {@link #setUTC} — called from
 *       the JavaFX thread (via {@link kepplr.commands.DefaultSimulationCommands}); safe because the
 *       anchor {@code AtomicReference} and {@code volatile pausedWallTime} provide the required
 *       memory visibility.
 * </ul>
 */
public final class SimulationClock {
     ...
     *
     * <p>{@link Double#NaN} means the simulation is running. A non-NaN value clamps {@link
     * #wallNow()} so ET stops advancing. {@code volatile} ensures the JME thread reads the value
     * written by the FX thread without a data race.
     */
    private volatile double pausedWallTime = Double.NaN;

    /**
     * ET at the end of the previous {@link #advance()} call — used to compute
     * {@code deltaSimSeconds}.
     *
    ...
    SimulationClock(DefaultSimulationState state, double startET, DoubleSupplier wallClock) {
        ...
        this.anchor = new AtomicReference<>(
                new TimeAnchor(startET, now, KepplrConstants.DEFAULT_TIME_RATE));
     ...
     *
     * <p>Reads the current anchor atomically, computes the new ET, and writes {@code currentEt}
     * and {@code deltaSimSeconds} to {@link DefaultSimulationState}. Must be called only from the
     * JME render thread (e.g., {@code simpleUpdate()}).
     */
    public void advance() {
     ...
     *
     * <p><b>Pause:</b> records the wall time at this moment; subsequent {@link #wallNow()} calls
     * return that clamped value so ET stops advancing.
     *
     * <p><b>Resume:</b> replaces the anchor atomically with {@code (etAtPause, pausedWallTime,
     * rate)}, then clears the pause latch. Using the pause wall time — not the resume wall time —
     * as the new {@code anchorWall} ensures that even if {@link #advance()} runs between the anchor
     * update and the volatile write, it still reads the correct frozen ET.
     *
     ...
     *
     * <p>Replaces the anchor atomically at the current {@link #wallNow()} moment so that ET
     * is continuous — no jump occurs at the boundary.
     *
     ...
     *
     * <p>Replaces the anchor atomically. The next {@link #advance()} call will begin advancing
     * from {@code et}.
     *
     ...
     *
     * @param utcString UTC time string in a format accepted by Picante's {@code utcStringToTDB}
     *                  (e.g., {@code "2015 Jul 14 07:59:00"})
     */
