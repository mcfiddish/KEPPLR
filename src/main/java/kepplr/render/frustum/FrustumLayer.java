 ...
 * {@link KepplrConstants#FRUSTUM_OVERLAP_FRACTION} (§8.2). The overlap is applied symmetrically:
 * the far plane of a layer extends 10% beyond the base boundary, and the near plane of the next
 * layer begins 10% before it.
 *
 * <p>Layers are ordered from most-local (NEAR) to most-distant (FAR). Assignment tries NEAR first
 * so bodies near the camera are always rendered in the most precise frustum available.
 */
public enum FrustumLayer {
     ...
     *
     * <p>Per §8.3: tries NEAR first, then MID, then FAR. If no single layer fully contains the
     * bounding volume (e.g., a very large body), returns the layer with the greatest containment
     * fraction.
     *
