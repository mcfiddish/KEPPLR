 ...
 *
 * <p>All stars are rendered in a single draw call using per-vertex buffers for position, size, and color. A
 * two-Gaussian core+halo GLSL fragment shader produces realistic star glows with additive blending.
 *
...
class StarFieldRenderer {
     ...
     *
     * <p>All previously attached geometries are detached first, then a new one is built and attached. All qualifying
     * stars are placed in a single mesh with per-vertex size and color data, rendered with a two-Gaussian core+halo
     * shader and additive blending.
     *
    ...
    void update(StarCatalog<? extends Star> catalog, double magnitudeCutoff, double et) {
        ...
        for (Star star : qualifying) {
            ...
            float pointSizePx = (float) clamp(
                    KepplrConstants.STAR_POINT_BASE_PX + KepplrConstants.STAR_POINT_SLOPE * log2flux,
                    1.0,
                    KepplrConstants.STAR_POINT_MAX_PX);
            float haloRadiusPx = (float) clamp(
                    KepplrConstants.STAR_HALO_BASE_PX + KepplrConstants.STAR_HALO_SLOPE * log2flux,
                    0.0,
                    KepplrConstants.STAR_HALO_MAX_PX);
